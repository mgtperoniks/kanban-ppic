<#
.SYNOPSIS
    Sync Production Database (kanban_ppic) to Local Laragon
    
.DESCRIPTION
    Script ini akan:
    1. SSH ke server production (10.88.8.46)
    2. Export database kanban_ppic dari container Docker (kanban-ppic-db)
    3. Download file backup ke lokal (C:\laragon\www\kanban-ppic\backups)
...
# Path di Server
$ServerAppPath = "/srv/docker/apps/kanban-ppic"
$ServerBackupDir = "/tmp"

# Konfigurasi Lokal
$LocalDbUser = "root"
$LocalDbPass = "123456788"
$RootDir = "C:\laragon\www\kanban-ppic"
$LocalBackupDir = "$RootDir\backups"
...
# FUNGSI
# ============================================
$DetailedLogFile = "$ScriptDir\sync_log_detail.txt"
...
# Target Sync (Aplikasi Kanban PPIC)
$Databases = @(
    @{ Name = "kanban_ppic"; LocalName = "kanban-ppic"; Container = "kanban-ppic-db"; User = "root" }
)

Log-Info "============================================"
Log-Info "   PRODUCTION TO LOCAL DB SYNC (KANBAN PPIC) "
Log-Info "============================================"
...
foreach ($Db in $Databases) {
    ...
    Log-Info ""
    Log-Info ">>> SINKRONISASI DATABASE: $DbName -> $LocalDbName"

    
    # 1. Export di Server (Direct Docker Exec)
    Log-Info "Mengekspor $DbName dari container $Container di server..."
    # Asumsi password root mysql di production adalah 123456788
    ...
    if ($LASTEXITCODE -ne 0) {
        Log-Error "Gagal ekspor $DbName. Pastikan database container berjalan dan password benar."
        $GlobalSuccess = $false
        continue
    }
    ...
    # 3. Import ke Lokal
    Log-Info "Menginput data ke MySQL Lokal (Laragon) database $LocalDbName..."
    if (Test-Path $localBackupPath) {
        ...
        $importCmd = "cmd /c `"`"$LaragonMysql`" -u$LocalDbUser -p$LocalDbPass $LocalDbName < `"$localBackupPath`"`""
