# Kanban PPIC - Safe Deployment Workflow
...
docker exec kanban_db mysqldump -u root -p[PASSWORD] kanban-ppic > ~/backup_before_update_$(date +%Y%m%d_%H%M%S).sql

# 4. Pull code terbaru
git pull origin main

# 5. Masuk ke container PHP dan jalankan migrasi
docker exec -it kanban_app php artisan migrate

# 6. Clear semua cache
docker exec -it kanban_app php artisan config:clear
docker exec -it kanban_app php artisan cache:clear
docker exec -it kanban_app php artisan view:clear
docker exec -it kanban_app php artisan route:clear

# 7. Jika ada perubahan di composer.json
docker exec -it kanban_app composer install --no-dev --optimize-autoloader

# 8. Jika ada perubahan di package.json / vite
docker exec -it kanban_app npm install
docker exec -it kanban_app npm run build
...
docker exec -i kanban_db mysql -u root -p[PASSWORD] kanban-ppic < ~/backup_before_update_XXXXXXXX_XXXXXX.sql
...
cp /srv/docker/apps/kanban-ppic/.env ~/env_backups/kanban-ppic.env.backup

# Atau download ke Windows (Gunakan PowerShell di local)
scp peroniks@peroniks-ppicserver:/srv/docker/apps/kanban-ppic/.env C:\laragon\www\kanban-ppic\backups\kanban-ppic.env.production
```
...
- [ ] Code sudah di-push ke github main
